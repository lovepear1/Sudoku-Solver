 ...
 * ProblemSet 5 
 * your name: Airi Hatori 
 * my email: airi0319@bu.edu
 * partner name: Hilal Bayram 
 * partner's email: senab@bu.edu
 */
...
public class Sudoku {    
    ...
    public Sudoku() {
        ...
        // set the entries in the extra bookkeeping arrays to true, 
    ...
    private boolean isValid(int val, int row, int col){
        if(this.subgridHasVal[row/3][col/3][val] == false 
            && this.colHasVal[col][val] == false && this.rowHasVal[row][val] == false 
    ...
    private boolean solveRB(int n) {
        // This boolean is the key recursive backtracking method and it 
        // should return true if a solution is found to the puzzle represented by the 
        // called Sudoku object, and false if no solution has yet been found. 
        // If there is more than one solution, your code should stoop after finidng 
        // one of them. this method takes a parameter n, which is the number of the cell that the current 
        // invocation of this method is responsible for. 
        int row = n / 9;
        int col = n % 9;
        if(n > 81 || n < 0){
        ...
        for(int val = 1; val <= 9; val++){
            if(this.isValid(val, row, col) == true){
